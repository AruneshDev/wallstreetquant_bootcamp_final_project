...
def load_benchmarks():
    """
    Download SOXX and SPY as benchmarks from 2020-01-01 to 2026-02-22.
    """
...
def alpha_decomposition(strat_ret: pd.Series,
    ...
    """
    Full-period Jensen's alpha, beta, Sharpe and related stats:
...
def strategy_correlation(cs_port: pd.Series,
    ...
    """
    Correlation matrix between strategies and benchmarks.
    """
    df = pd.DataFrame({
        'CS Momentum': cs_port,
...
def yearly_alpha(strat_ret: pd.Series,
    ...
    Keeps all years (including partial years like 2026).
    """
    rf_daily = (1 + rf_annual) ** (1 / 252) - 1
...
if __name__ == "__main__":
    ...
    results = []
    results.append(alpha_decomposition(cs_port, soxx,
                                       label="CS Momentum (45d) vs SOXX"))
    results.append(alpha_decomposition(cs_port, spy,
                                       label="CS Momentum (45d) vs SPY"))
    results.append(alpha_decomposition(pairs_port, soxx,
                                       label="QCOM/MRVL Pairs vs SOXX"))
    results.append(alpha_decomposition(pairs_port, spy,
                                       label="QCOM/MRVL Pairs vs SPY"))
    ...
    # ── Year-on-year alpha ───────────────────────────────────
    cs_yearly_soxx = yearly_alpha(cs_port, soxx,
                                  label="CS Momentum (45d) vs SOXX")
    cs_yearly_spy = yearly_alpha(cs_port, spy,
                                 label="CS Momentum (45d) vs SPY")

    p_yearly_soxx = yearly_alpha(pairs_port, soxx,
                                 label="QCOM/MRVL Pairs vs SOXX")
    p_yearly_spy = yearly_alpha(pairs_port, spy,
                                label="QCOM/MRVL Pairs vs SPY")
