Here is the complete `README.md` — save it to your project root:
...
Tested all 272 directed pairs across 17 tickers (785 trading days).

```
Result: Zero pairs with positive lift at any lag 1–5d
Max lift observed: –0.20
Conclusion: Daily lead-lag alpha REJECTED
```

This is the most important finding of Step 2. Most practitioners assume NVDA
leads — the data says it does not at daily resolution. Same-day correlation is
the dominant signal (r ≈ 0.60–0.75), which is not exploitable after costs.

### Step 3 — Pivot to Momentum
A full robustness sweep across 8 lookback windows revealed a clean
...
### Step 4 — ML Layer
Four models evaluated on identical OOS test data
(Apr 2025 – Feb 2026, 214 days):
...
(IC > 0.05). Graph structure — correlation-weighted edges between stocks —
captures sector contagion dynamics that flat-feature models cannot encode.
...
| 2025 | –12.8%    | –0.522 | Regime break — DeepSeek shock, rotation |
| 2026 | +21.2%    | 4.576  | Only 34 days — not statistically meaningful |

### Strategy 2 — NVDA / TXN Pairs Trade
Log-spread mean reversion. Entry at ±1.5σ, exit at ±0.3σ, 120-day rolling
...
Combining both strategies diversifies the 2025 momentum drawdown while
preserving the pairs trade's consistency.
...
vol_ratio                                    ← vol_5d / vol_20d regime
...
dominate — microstructure and trend signals, not momentum rank.

### GNN Architecture
```
Input : node features (12 stocks × 11 features)
Adj   : D^{-1/2} (A + I) D^{-1/2}  — correlation-weighted, threshold=0.3
        60-day rolling window, recomputed daily
Layer 1: GCNLayer(11 → 32) + ELU + LayerNorm + Dropout(0.1)
Layer 2: GCNLayer(32 → 32) + ELU + LayerNorm
...
Encoder    : 2 × TransformerEncoderLayer(d=32, heads=4, ff=128, pre-norm)
Head       : LayerNorm → Dropout → Linear(32 → 1)
...
│
├── results/                      # All CSV outputs (git-tracked)
│   ├── cs_momentum_robustness.csv
│   ├── cs_momentum_annual.csv
│   ├── cs_momentum_monthly.csv
│   ├── pairs_annual.csv
│   ├── pairs_monthly.csv
│   ├── rf_ic.csv / rf_rank_ic.csv
│   ├── gbm_ic.csv / gbm_rank_ic.csv
│   ├── transformer_ic.csv / transformer_rank_ic.csv
│   ├── gnn_ic.csv / gnn_rank_ic.csv
...
python src/backtest.py           # run strategies
python src/plots.py              # generate charts
python src/model_baseline.py     # RF + GBM IC
python src/model_transformer.py  # Transformer IC
python src/model_gnn.py          # GNN IC (best model)
...
- **Universe**: 12 semiconductors (NVDA, AMD, AVGO, TSM, QCOM, AMAT, LRCX,
  MU, KLAC, TXN, ASML, MRVL) + 5 big tech (AAPL, MSFT, GOOGL, META, AMZN)
...
1. **Short history (2 years)**: Walk-forward IC estimates have wide confidence
   intervals. A 5–10 year backtest on a longer dataset is required before
   live deployment.

2. **Transaction costs not modelled**: CS Momentum rebalances daily across
   6 legs. At realistic bid-ask spreads for mid-cap semis (~5–10bps per leg),
   the strategy's net Sharpe would decline meaningfully.

3. **Train/test sample imbalance (Transformer)**: With only 321 training days
   and a 20-day sequence length, the Transformer had limited samples to learn
   from. Longer data or transfer learning from a broader equity universe would
   improve IC.

4. **GNN correlation window**: The 60-day rolling adjacency matrix introduces
   look-ahead-free but lagged structural information. During fast regime
   changes (e.g. DeepSeek shock Feb 2025), the graph may reflect stale
   correlations for up to 60 days.

5. **2025 regime break**: CS Momentum Sharpe dropped from +1.15 (2024) to
   –0.52 (2025), confirming that momentum strategies are regime-dependent.
   Regime detection (e.g. HMM or rolling Sharpe filter) would improve
...
- [ ] Regime detection overlay — reduce CS Momentum exposure when rolling
      63d Sharpe < 0
- [ ] GNN signal as position-sizing multiplier on CS Momentum
- [ ] Expand universe to 50+ semis for more statistically reliable IC
- [ ] Add alternative data: earnings surprise, analyst revision momentum,
      SOXX ETF flow
- [ ] Transaction cost model + realistic slippage simulation
- [ ] Cointegration test across broader pairs universe (not just NVDA/TXN)
